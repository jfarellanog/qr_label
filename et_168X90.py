...
for index, row in ubicaciones.iloc[0:].iterrows():
        ...
        code = row['Posicion']
        img = Image.new('RGB', (325,325), color=(255, 255, 255))
        fnt = ImageFont.truetype(r'C:\Users\jarellan\Desktop\qr_label\IN\Futura Bold font.ttf', 70)
        d = ImageDraw.Draw(img)

        qr_big = qrcode.QRCode(error_correction=qrcode.constants.ERROR_CORRECT_H, box_size=11, border=2)
        qr_big.add_data(code)
        qr_big.make()
        img_qr_big = qr_big.make_image().convert('RGB')
        img.paste(img_qr_big, (10, 10))
        ...
        img.save('C:\\Users\\jarellan\\Desktop\\qr_label\\OUT\\MODELO\\' + row['Posicion'] + '_' + str(id_etiqueta + k) + '.jpg', dpi=(300, 300))
        k += 1
