import qrcode
from PIL import Image, ImageDraw, ImageFont
import numpy as np
import pandas as pd
import xlsxwriter
import openpyxl as xl
from datetime import datetime
from openpyxl import load_workbook

excel_file = r'C:\Users\jarellan\Desktop\qr_label\IN\SKU_PLASCO'
ubicaciones = pd.read_excel(excel_file+".xlsx")

ubicacion = []
fecha = datetime.today().strftime("%d-%m-%Y")
k=1
id_etiqueta = 0
imgs_v2 = []
for index, row in ubicaciones.iloc[0:].iterrows():
        images = []
        # Con crop corto la imagen
        # face = Image.open('logo_ccu.png')
        code = row['Posicion']
        sku = str(row['SKU'])
        abr = str(row['Descr1'])
        abr2 = str(row['Descr2'])
        if abr2 == 'nan':
                abr2=""
        if abr == 'nan':
                abr = ""
        img = Image.new('RGB', (325,150), color=(255, 255, 255))
        fnt = ImageFont.truetype(r'C:\Users\jarellan\Desktop\qr_label\IN\Futura Medium Italic font.ttf', 15)
        fnt2 = ImageFont.truetype(r'C:\Users\jarellan\Desktop\qr_label\IN\Futura Bold font.ttf', 30)
        d = ImageDraw.Draw(img)
        d.text((5, 5), str(code), font=fnt2, fill=(0, 0, 0))
        d.polygon([(215, 20),(215,30),(225, 25)],  fill=(0, 0, 0))
        d.text((5, 50), str(sku), font=fnt2, fill=(0, 0, 0))
        d.text((5,90), str(abr), font=fnt, fill=(0, 0, 0))
        d.text((5, 120), str(abr2), font=fnt, fill=(0, 0, 0))
        d.polygon([(215, 120), (215, 130), (225, 125)], fill=(0, 0, 0))

        qr_big = qrcode.QRCode(error_correction=qrcode.constants.ERROR_CORRECT_H, box_size=2, border=1)
        qr_big.add_data(code)
        qr_big.make()
        img_qr_big = qr_big.make_image().convert('RGB')
        img.paste(img_qr_big, (240, 5))

        qr_big = qrcode.QRCode(error_correction=qrcode.constants.ERROR_CORRECT_H, box_size=2, border=1)
        qr_big.add_data(sku)
        qr_big.make()
        img_qr_big = qr_big.make_image().convert('RGB')
        img.paste(img_qr_big, (240, 100))

        # save that beautiful picture
        id_etiqueta += 1
        #imgs_comb.save('C:\\Users\\jarellan\\Desktop\\qr_label\\OUT\\PLASCO\\etiqueta' + row['Posicion'] + '_' + str(id_etiqueta + k) + '.jpg', dpi=(300, 300))
        print('Impresión de ' + row['Posicion'] + '_' + str(k))
        imgs_v2.append(img)
        # df.loc[last_row]=[str(code),str(rectangle),str(circle),str(triangle),int(id_etiqueta + k),row['Posicion'] + '_' + str(id_etiqueta + k)]
        # print(df)
        if k % 2 == 0:
                img_vf = Image.new('RGB', (325, 325), color=(255, 255, 255))
                d2 = ImageDraw.Draw(img_vf)
                posv_1 = (0, 0)
                posv_2 = (0, 150)
                img_vf.paste(imgs_v2[0], posv_1)
                d2.line((0, 150, 325, 150), fill=(0, 0, 0), width=3)
                img_vf.paste(imgs_v2[1], posv_2)
                d2.line((0, 300, 325, 300), fill=(0, 0, 0), width=3)
                imgs_v2.clear()
                img_vf.save('C:\\Users\\jarellan\\Desktop\\qr_label\\OUT\\PLASCO\\' + row['Posicion'] + '_' + str(id_etiqueta + k) + '.jpg', dpi=(300, 300))
        k += 1
